...
const Footer = () => {
   ...
   useEffect(() => {
      clientSocket.open();
      const handlePurchaseNotification = async () => {
         try {
            await refetchCart();
            await refetchNoti();
         } catch (err) {
   ...
   const handleRemoveProductInCart = (item: ICartDataBaseItem | ICartItems) => {
      if (auth.user?._id) {
         deleteProductInCartDB(item?.productId?._id).then(() => {
   ...
   return (
      <>
         <section
            ...
            className=' section-icon-contact fixed bottom-[30px] right-[24px] cursor-pointer z-[4]'
         >
         ...
         <section
            ...
            className='chat max-sm:max-w-[316px] max-sm:bottom-[71px] max-md:right-[3px] max-w-[450px] min-w-[300px] rounded-t-md fixed bottom-[0px] right-[100px] shadow-[0_12px_28px_0_rgba(0,0,0,0.1)] z-[999] bg-white h-[455px]'
         >
            <div className='header-right justify-between rounded-t-md mb-[10px] bg-white  sticky top-[0] pl-[10px] flex items-center  shadow-[0_0_4px_rgba(0,0,0,0.2)] py-[5px]'>
               <div className='user ifo flex items-center gap-x-[10px]'>
                  <img
                     ...
                     alt=''
                  />
                  <span className='user-name text-black font-bold '>Admin</span>
               </div>
               ...
               <ul className='footer-list flex justify-between py-[60px] flex-wrap ml-[-30px]'>
                  <li className='footer-if  w-full lg:w-[calc(35%-30px)] ml-[30px]'>
                     ...
                     <div className='footer-if-text leading-7'>
                        ...
                        <p className='mt-2'>
                           <span>
                              Email: <a href={'mailto:' + info.email} rel='noreferrer' target='_blank' className='hover:underline'>{info.email}</a>
                           </span>
                        </p>
                        <p className='mt-2'>
                           <span>
                              Website: <a href={info.websiteUrl} rel='noreferrer' target='_blank' className='hover:underline'>{info.websiteName}</a>
                           </span>
                        </p>
                     </div>
                     <div className='call-line mt-[30px] flex items-center'>
                        ...
                        <div className='call-if'>
                           <p className='call-title text-redTextColor text-[20px] font-bold'>Số HOTLINE:</p>
                  ...
                  <li className='footer-if list-link-instar ml-[30px] w-full lg:w-[calc(23%-30px)] max-lg:mt-[20px] '>
                     <div className='ft-title font-bold text-redTextColor text-[18px] mb-[5px] relative'>
                        Theo dõi trên FaceBook
                     </div>
                     <iframe src="#TODO add later" width="340" height="331" style={{ border: "none", overflow: "hidden" }} scrolling='no' allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"></iframe>
                  </li>
               </ul>
            </div>
            <div>
               <p className='text-center py-4'>2024 © Copyright NƯỚC MẮM PHÚ NGHĨA. Designed by Nina</p>
            </div>
         </footer>
               ...
               <div className='wrap-mini-cart transition-all duration-300 translate-x-[100%] w-[320px] flex h-full fixed z-[99] top-0 right-0 flex-col bg-white text-[#6f6f6f]'>
                  <div className='mini-cart-header flex border-b-[#e2e2e2] border-[1px]'>
                     <p className='cart-header-text w-full gap-[10px] py-[10px] px-[15px] flex items-center  text-[14px]'>
                        <span className='cart-count px-[8px] text-[14px] py-[4px] text-white bg-redTextColor'>
                           {cart?.length}
                        </span>
                     ...
                     <ul className='cart-item relative'>
                              ...
                              >
                                 ...
                                 <div className='cart-content w-[calc(100%-65px)] pl-[15px] flex flex-col justify-center'>
                                    ...
                                    <div className='product-info mt-[9px] flex'>
                                       <span className='product-qt text-[16px]'>{item?.weight} ×</span>
                        ...
                        <div className='start-shopping cart-title  border-[#e2e2e2] gap-2 font-bold flex justify-center items-center text-center pb-[12px]'>
                           <Link
                              to={'/collections'}
                              onClick={showMiniCart}
                              className='block  xl:text-[14px] max-xl:text-[14px] view-cart w-[40%] transition-all duration-300 hover:bg-color1st rounded-[10px] py-[10px] px-[30px] bg-color3rd text-white text-center mb-[20px]'
                           >
                              Mua hàng
                           </Link>
                           <Link
         ...
         <section className='user-tag-moble w-[320px]'>
            ...
            <div className='user-tag-mobile-content transition duration-300 fixed top-0 left-0 h-full bg-white z-[8] min-w-[320px] translate-x-[-100%]'>
               <ul>
                  ...
                  {!auth.accessToken && (
                     <>
                        <li onClick={showUserTag} className='px-[15px] py-[10px] hover:bg-colortext-color3rd hover:text-white'>
                  ...
                  {auth.accessToken && (
                     <>
                        {auth.user?.role == 'admin' && (
                           <li
                              onClick={showUserTag}
                              className='px-[15px] py-[10px] hover:bg-colortext-color3rd hover:text-white'
                           >
