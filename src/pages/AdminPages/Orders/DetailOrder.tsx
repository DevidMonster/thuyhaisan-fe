...
import { useUpdateOrderMutation, useUpdatePayStatusMutation } from '../../../services/order.service';
...
const DetailOrder = ({ idOrder }: Props) => {
   ...
   const [handleUpdatePayStatus, { isLoading: loading }] = useUpdatePayStatusMutation();
   ...
   const handleChangePaymentStatus = async (value: boolean): Promise<void> => {
      if (!order || loading) return;

      try {
         await handleUpdatePayStatus({
            idOrder,
         ...
         adminSocket.emit(
            ...
            JSON.stringify({
               userId: order.userId!,
               orderId: idOrder,
               status: "Đã cập nhật trang tháu thanh toán",
   ...
   return (
      <div className='p-3 max-h-[800px] overflow-auto'>
         ...
         {(statusOrder !== FAIL_ORDER.toLowerCase()) &&
            <>
               <Row className='py-3 border-t-[1px] border-[rgba(0,0,0,0.1)] mt-5' align={'middle'}>
                  <Col span={6}>
                     <span className="text-sm  text-greenP800 font-bold"> TRẠNG THÁI THANH TOÁN:</span>
                  </Col>
                  <Col span={6}>
                     <ButtonCheck
                        colorPrimary={"#80b235"}
                        value={"Chưa thanh toán"}
                        disable={order?.pay === false}
                        onClick={() => {
                           handleChangePaymentStatus(false)
                        }}
                     />
                  </Col>
                  <Col span={6}>
                     <ButtonCheck
                        colorPrimary={"#80b235"}
                        value={"Đã thanh toán"}
                        disable={order?.pay === true}
                        onClick={() => {
                           handleChangePaymentStatus(true)
                        }}
                     />
                  </Col>
               </Row>
