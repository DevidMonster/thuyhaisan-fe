...
type Props = {
   products: IProduct[];
   submitProduct: (data: ProductInput, index?: number) => void;
   removeProduct: () => void;
   data: ProductInput;
   itemIndex: number;
...
const FormProduct = ({ products, submitProduct, data, removeProduct, productData, itemIndex }: Props) => {
   const { Option } = Select;
   ...
   const handleAddProduct = (data: ProductInput, type: 'save' | 'autosave', itemIndex: number) => {
      const UNIQE_COUNT = 1;
      if (type === 'save') {
      ...
      submitProduct(newData, itemIndex);
   };
   useEffect(() => {
   ...
   useEffect(() => {
      ...
      const timeId = setTimeout(() => {
         ...
         handleAddProduct(newData, 'autosave', itemIndex);
         setIsSave(true);
      }, 800);
   ...
   return (
      <Form
         form={formProduct}
         onFinish={(value) => handleAddProduct(value, 'save', itemIndex)}
      ...
      >
         <div className='flex justify-stretch items-center gap-3'>
            ...
            >
               <Input type='number' prefix='vnd/sản phẩm|' min={'1'} onChange={() => setIsSave(false)} />
            </Form.Item>
         </div>
         <div className='flex justify-stretch items-center gap-3'>
            <Form.Item
            ...
            >
               <Input type='number' min={'1'} onChange={() => setIsSave(false)} />
            </Form.Item>
            <Form.Item
               className='w-[50%]'
               label='Hạn sử dụng'
         ...
         <Space size={'large'} direction='horizontal' className='mb-[24px]'>
            <Popconfirm title='Bạn muốn xóa sản phẩm này ?' onConfirm={() => handleConfirm()}>
               <Tooltip placement='top' title='Hủy'>
                  ...
                  >
                     <span className='text-color3rd hover:!bg-color1st  w-4 font-bold'>x</span>
                  </button>
               </Tooltip>
            </Popconfirm>
            {!isSave && (
               <Form.Item className='mb-0'>
                  <Tooltip placement='top' title='Lưu'>
                     <button
                        type='submit'
                        className='flex justify-center items-center p-2 rounded-full bg-color3rd hover:!bg-color1st  w-10 h-10 shadow-md hover:w-11 hover:h-11 duration-100 '
                     >
                        <ConfirmIcon className='text-color3rd hover:!bg-color1st  w-4 fill-white' />
                     </button>
                  </Tooltip>
               </Form.Item>
            )}
