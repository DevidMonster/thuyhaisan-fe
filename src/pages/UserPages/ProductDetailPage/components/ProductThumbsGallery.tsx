      ...
      <>
         ...
         >
            {body?.map((item, index) => {
               return (
                  <SwiperSlide
                     className='border  sm:h-[527px] max-sm:min-h-[400px] border-[#e2e2e2] min-h-[400px]'
                     key={item.public_id + index}
                  >
                     <img className='w-full h-full rounded-[5px]' alt={item.public_id} src={item.url} />
                  </SwiperSlide>
               );
            })}
         </Swiper>
         ...
         >
            {body?.map((item, index) => {
               return (
                     <SwiperSlide key={index} className='cursor-pointer w-[115px] h-[140px]'>
                        <img alt="" className=' h-full w-full  rounded-[5px]' src={item.url} />
                     </SwiperSlide>
               );
            })}
         </Swiper>
