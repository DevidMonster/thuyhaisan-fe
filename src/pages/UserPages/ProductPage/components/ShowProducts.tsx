...
interface IProps {
   ...
   filterByCateId?: string
}
const ShowProducts = ({ data, filterByCateId }: IProps) => {
      ...
      <div>
         <div className='list-products grid xl:grid-cols-3 pt-[30px] lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-2 max-sm:grid-cols-2  max-md:gap-[12px]'>
            {data?.body.data.map((item, index) => {
               if(filterByCateId && item.categoryId._id !== filterByCateId) return <Fragment key={index}></Fragment>
               return (
                  <div key={index} className='product-item max-w-[290px] mx-auto md:p-[10px]  max-xl:mb-[18px]'>
                     <div className='product-wrap h-full overflow-hidden group/product-wrap relative flex flex-col justify-between   max-xl:pb-[40px]'>
                        {item.discount > 0 && (
                        ...
                        {item.shipments.length <= 0 && (
                           <span className='discount z-[1] transition-all duration-300 group-hover/product-wrap:translate-x-[115%] bg-red-500 min-w-[40px] text-center absolute rounded-[3px] py-[5px] px-[10px] text-[12px] text-white right-[7px] top-[7px]'>
                              Hết hàng
                           </span>
                        )}
                        {item.isSale == true && (
                        ...
                        <div className='wrap-product-img overflow-hidden xl:relative max-xl:text-center '>
                           <div className='xl:relative product-img   after:absolute after:top-0 after:left-0 after:right-0 after:bottom-0 bg-[#ffffff] after:opacity-0 after:invisible transition-all duration-300 group-hover/product-wrap:visible xl:group-hover/product-wrap:opacity-[0.4] max-xl:group-hover/product-wrap:opacity-[0.5] '>
                              <Link to={'/products/' + item._id}>
                                 <img
                                    className='product-main-img lg:!h-[350px] w-[full] md:!h-[270px] sm:!h-[290px] max-sm:h-[170px] xl:group-hover/product-wrap:invisible  visible transition-all duration-300 opacity-100 object-cover object-center'
                                    src={item?.images[0]?.url}
                                    alt=''
                                 />
                                 <img
                                    className='product-sub-img lg:!h-[350px] w-[full] md:h-[270px] sm:h-[290px] max-sm:h-[170px] max-xl:hidden absolute group-hover/product-wrap:opacity-100 group-hover/product-wrap:visible transition-all duration-300 top-0 left-0 invisible opacity-0  object-cover object-center'
                           ...
                           <div className='product-action max-xl:w-full max-xl:justify-center  transition-all duration-300 xl:invisible xl:opacity-0 flex absolute xl:bottom-[50%] bottom-0 xl:right-[50%] xl:translate-x-[50%] xl:gap-[15px]  max-xl:gap-[10px] group-hover/product-wrap:opacity-100 group-hover/product-wrap:visible'>
                              <button
                                 ...
                                 className='add-to-card flex items-center justify-center transition-all duration-300 cursor-pointer hover:bg-color1st w-[40px] h-[40px] text-[20px] rounded-[100%] text-white bg-color3rd'
                              >
                                 <HiOutlineShoppingBag></HiOutlineShoppingBag>
                              </button>
                              <button
                                 onClick={() => openQuickViewModal(item)}
                                 className='flex items-center justify-center transition-all duration-300 cursor-pointer hover:bg-color1st w-[40px] h-[40px] text-[20px] rounded-[100%] text-white bg-color3rd'
                              >
                        ...
                        >
                           <p className='product-name font-bold md:mt-[10px] text-center md:text-[18px] max-md:text-[16px] line-clamp-2 break-words hover:text-color1st'>
                              {item?.productName}
                        ...
                        <div className=''>
                           ...
                           <p className='price mt-[9px] flex items-center justify-center  text-center font-bold md:mb-[20px] max-md:mb-[10px] md:text-[18px]  text-color3rd'>
