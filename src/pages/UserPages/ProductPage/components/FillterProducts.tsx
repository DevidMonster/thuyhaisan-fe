   ...
   return (
      <div className='main-header-filter lg:mt-[20px] lg:bg-[#f8f8f8] max-lg:!mt-[-120px] max-lg:flex max-lg:flex-col max-lg:pb-[50px]  overflow-y-auto max-lg:bottom-0 max-lg:translate-y-[130%] transition-transform duration-500 max-lg:right-0 max-lg:left-0  lg:mx-[-15px] lg:sticky w-[25%] top-[120px]  max-lg:w-[100%] max-lg:fixed max-lg:z-[13] bg-white  max-md:p-0 '>
            ...
            >
               ...
               <div className='list-categories p-[10px]  border-t-[1px] border-[#eae4e8] gap-y-[20px] flex max-lg:gap-y-[15px] flex-col max-lg:flex-wrap max-lg:flex-row  '>
                  <div
                     ...
                     style={{
                        background: filter.field.category == '' ? '#0271ba' : '',
                        color: filter.field.category == '' ? 'white' : '',
                        borderRadius: filter.field.category == '' ? '2px' : ''
                     }}
                     className='max-lg:w-[25%] max-lg:flex max-lg:flex-col max-lg:items-cente hover:text-color1st py-2 px-3'
                  >
                     <button type='button'>
                  ...
                  {data?.body.data.map((item, index) => {
                     return (
                        <div
                           ...
                           style={{
                              background: filter.field.category == item._id ? '#0271ba' : '',
                              borderRadius: filter.field.category == item._id ? '2px' : '',
                              color: filter.field.category == item._id ? 'white' : ''
                           }}
                           className='max-lg:w-[25%] max-lg:flex max-lg:flex-col max-lg:items-center  hover:text-color1st py-2 px-3'
                        >
         ...
         <div className='main-header-bottom flex justify-between lg:hidden bg-white fixed bottom-0 px-[5px] pt-[10px] py-[12px] border-t-[1px] border-[#eae4e8] w-full'>
            <button
               onClick={showFilter}
               className='text-[13px] font-[500] bg-[#f3f4f6] border-[1px] border-[#e5e7eb] text-center py-[10px] px-[15px] cursor-pointer block rounded-[2px] outline-none w-[calc(50%-12px)]'
            >
               HUỶ
            </button>
            <button onClick={showFilter} className='text-[13px] text-white font-[500] bg-color3rd hover:bg-color1st border-[1px] border-[#e5e7eb] text-center py-[10px] px-[15px] cursor-pointer block rounded-[2px] outline-none w-[calc(50%-12px)] '>
               ÁP DỤNG
            </button>
         </div>
      </div>
