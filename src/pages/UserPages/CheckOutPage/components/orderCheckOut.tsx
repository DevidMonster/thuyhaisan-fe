...
const PAYMENT_METHODS: paymentMethod[] = [
   ...
   { name: 'Thanh toán online', value: 'online', type: 'text' },
];
const OrderCheckOut = ({ onSubmit, methods, loadingState }: Iprops) => {
   const [PayValue, setPayValue] = useState<'cod' | 'online'>('cod');
         ...
         <div className='order-checkout'>
            <div className='order-checkout-content  flex max-md:flex-wrap  ml-[-30px]'>
               <div className='check-pro ml-[30px] md:w-[calc(50%-30px)] max-md:w-full '>
                  <span className='text-[26px] text-redTextColor font-bold'>
                                 ...
                                 <li className='check-pro-item flex items-center mb-[20px] pb-[20px] border-b border-[#e2e2e2]'>
                                    ...
                                    <div className='check-pro-content ml-[15px] w-[calc(100%-112px)]'>
                                       <Link
                                          ...
                                          className='block font-bold text-redTextColor'
                                       >
               ...
               <div className='order-detail md:w-[calc(50%-30px)] max-md:w-full  ml-[30px]'>
                  <span className='text-[20px] text-redTextColor font-bold'>Đơn hàng của bạn</span>
                                 ...
                                 {!haveIsSale ? PAYMENT_METHODS.map((method) => (
                                    ...
                                    <Radio value={method.value}>
                                       {method.type === 'logo' ? (
                                          ...
                                          <span className='text-color3rd hover:!bg-color1st  font-semibold'>{method.name}</span>
                                       )}
                                    </Radio>
                                    ...
                                    {PAYMENT_METHODS[1].type === 'logo' ? (
                                       ...
                                       <span className='text-color3rd hover:!bg-color1st  font-semibold'>{PAYMENT_METHODS[1].name}</span>
                                    )}
                                 </Radio>}
                              </Radio.Group>
                              {PayValue === 'online' &&
                                 <div>
                                    <div className="transition-all w-full mt-[5px] py-[10px] px-[15px] bg-[#f5efef]">
                                       SỐ TK: 114604228686  - Ngân hàng TMCP công thương Việt Nam (Viettin Bank) – CN BÃI NGANG , QUỲNH LƯU, N.A
                                       <br />CÔNG TY CP SXKD TỔNG HỢP THỦY HẢI SẢN PHÚ NGHĨA
                                    </div>
                                 </div>
                              }
                           </ConfigProvider>
                  ...
                  <div
                     style={{ backgroundColor: confirm == false ? 'gray' : '#0271ba' }}
                     className='wrap-btn-order-detail  mt-[28px] text-center text-[18px]  transition-colors duration-300 hover:bg-color1st rounded-[10px] '
                  >
                     <Button
